...
- **中转**：把入口服务器的一个端口转发到落地服务器或任意地址（realm，TCP 和 UDP 都能转），落地机上的节点不用改。入口机的监听端口由面板自动放行，删除中转时只收回面板自己开的那条，你手动开的规则不动；这一跳还可以选择用 TLS 加密，客户端不用改任何配置。列表直接显示延迟、抖动、丢包和已搬运的流量，点开某一行还有 1 小时到 7 天的曲线图（psm-agent 每分钟测一次，用 TCP 连接而不是 ping，面板保留 7 天）。
...
- 管理员密码来自 Worker 机密 `ADMIN_PASSWORD`；面板第一次读到它时，会把**加盐的 PBKDF2 哈希**（不是明文，也不是可离线快速爆破的单轮摘要）存进 D1，这样重新部署丢了机密也不会被锁在门外。机密还在时以机密为准，改机密即改密码。登录会话是签名的 HttpOnly Cookie，同一地址 15 分钟内失败 10 次后暂停登录。
